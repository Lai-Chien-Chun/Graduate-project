...
data=data.append([data2,data3],ignore_index=True)
print(data)
...
#為[-1,8,1]
#---------------------------
with tf.name_scope('Input_Layer'):#設定計算圖的名稱
	X_data = tf.placeholder(tf.float32,shape=[None,8])
	X = tf.reshape(X_data,[-1,8,1])
	print("x:",X.shape)
...
with tf.name_scope('Conv1_Layer'):
	...
	Conv1 = tf.nn.conv1d(X,W1,stride=1,padding = 'SAME')+b1
	C1 = tf.nn.relu(Conv1)
	print("C1:",C1.shape)
